...
def test_extract_words_basic():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        results = extract_words(merged, pp, trie, total,
                                pmi_threshold=0, entropy_threshold=0, pos_threshold=0)
...
def test_extract_words_pos_threshold():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert len(extract_words(merged, pp, trie, total,
                                 pmi_threshold=0, entropy_threshold=0, pos_threshold=0.2)) == 0
        assert len(extract_words(merged, pp, trie, total,
                                 pmi_threshold=0, entropy_threshold=0, pos_threshold=0.05)) == 1
...
def test_pipeline_utf8():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _saved = (cfg.ENTROPY_THRESHOLD, cfg.PMI_THRESHOLD, cfg.POS_PROB_THRESHOLD)
        ...
        try:
            ...
            cfg.ENTROPY_THRESHOLD, cfg.PMI_THRESHOLD, cfg.POS_PROB_THRESHOLD = _saved
